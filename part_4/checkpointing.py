...
def _log_model_stats(logger, model, step: int, do_hists: bool = False):
    ...
    try:
        ...
        total_param_norm = torch.norm(
            torch.stack([p.detach().norm(2) for p in params]), 2
        ).item()
    ...
    try:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            ...
            for li, blk in enumerate(getattr(model, "blocks", [])):
                ...
                q = (
                    attn.wq(h).view(B, T, attn.n_head, attn.d_head).transpose(1, 2)
                )  # (B,H,T,D)
                k = (
                    attn.wk(h).view(B, T, attn.n_kv_head, attn.d_head).transpose(1, 2)
                )  # (B,Hk,T,D)
                v = (
                    attn.wv(h).view(B, T, attn.n_kv_head, attn.d_head).transpose(1, 2)
                )  # (B,Hk,T,D)
                ...
                if q1.numel() > 0:
                    logger.hist(f"qkv/block{li}/q_hist", q1, step)
                if k1.numel() > 0:
                    logger.hist(f"qkv/block{li}/k_hist", k1, step)
                if v1.numel() > 0:
                    logger.hist(f"qkv/block{li}/v_hist", v1, step)
                ...
                if q1.numel():
                    logger.log(
                        step=step,
                        **{
                            f"qkv/block{li}/q_l2_mean": float(q1.square().mean().sqrt())
                        },
                    )
                if k1.numel():
                    logger.log(
                        step=step,
                        **{
                            f"qkv/block{li}/k_l2_mean": float(k1.square().mean().sqrt())
                        },
                    )
                if v1.numel():
                    logger.log(
                        step=step,
                        **{
                            f"qkv/block{li}/v_l2_mean": float(v1.square().mean().sqrt())
                        },
                    )
...
def _log_samples_tb(
    logger, model, tok, xb, device, step: int, max_new_tokens: int = 64
):
    if not _is_tb(logger):
        return
    if tok is None:
...
def _extract_config_from_model(model) -> dict:
    ...
    try:
        ...
        cfg["n_kv_head"] = int(
            getattr(attn, "n_kv_head", cfg["n_head"])
        )  # default to MHA
    ...
    got.update(
        {
            ...
            "n_kv_head": int(
                getattr(first_blk.attn, "n_kv_head", first_blk.attn.n_head)
            ),
        }
    )
    diffs = [
...
def save_checkpoint(
    ...
    if hasattr(model, "config"):
        ...
        cfg = (
            ...
            else getattr(cfg_obj, "__dict__", None) or _extract_config_from_model(model)
        )
    else:
    ...
    torch.save(
        {
            ...
            "amp_scaler": (
                ...
                if amp and getattr(amp, "scaler", None)
                else None
            ),
            "step": int(step),
...
def load_checkpoint(
    model, path: str, optimizer=None, scheduler=None, amp=None, strict: bool = True
):
    ...
    if (
        amp is not None
        and ckpt.get("amp_scaler") is not None
        and getattr(amp, "scaler", None)
    ):
...
def atomic_save_all(
    ...
    save_checkpoint(
        model, optim, sched, amp, step, str(out_dir), tok_dir, config=config
    )  # writes model_last.pt
