...
def score_policy(policy_ckpt: str, rm_ckpt: str, bpe_dir: str | None, n: int = 16):
    ...
    ckpt_ref = torch.load(
        "../part_6/runs/sft-demo/model_last.pt", map_location=device
    )  # hardcoded path to SFT checkpoint
    ...
    for p in prompts:
        ...
        z = torch.tensor(
            [tok.encode(text)[: tok.block_size]], dtype=torch.long, device=device
        )
        with torch.no_grad():
            r = rm(z)[0].item()
        rewards.append(r)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--split", type=str, default="val[:32]"
    )  # unused in this tiny script
