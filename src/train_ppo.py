...
from rollout import RLHFTokenizer, sample_prompts
from rollout import model_logprobs
from formatters import format_prompt_only, format_example, Example
...
def compute_reward(
    reward_model: RewardModel, tok: RLHFTokenizer, prompt: str, response: str, device
) -> float:
...
def main():
    ...
    p.add_argument(
        "--policy_ckpt", type=str, required=True, help="SFT checkpoint (Part 6)"
    )
    p.add_argument(
        "--reward_ckpt",
        type=str,
        required=True,
        help="Reward model checkpoint (Part 7)",
    )
    ...
    while step < args.steps:
        ...
        batch_prompts = prompts[
            ...
            % len(prompts) : ((step + 1) * args.batch_size)
            % len(prompts)
        ]
        ...
        with torch.no_grad():
            ...
            for i, x in enumerate(in_ids):
                ...
                out = policy.generate(
                    idx, max_new_tokens=args.resp_len, temperature=0.2, top_k=3
                )
