...
class CausalSelfAttentionModern(nn.Module):
    def __init__(
        ...
        ), "n_head must be multiple of n_kv_head (GQA grouping)"
    ...
    def forward(
        self, x: torch.Tensor, kv_cache: KVCache | None = None, start_pos: int = 0
    ):
        ...
        # Projections
        q = (
            self.wq(x).view(B, T, self.n_head, self.d_head).transpose(1, 2)
        )  # (B,H, T,D)
        k = (
            self.wk(x).view(B, T, self.n_kv_head, self.d_head).transpose(1, 2)
        )  # (B,Hk,T,D)
        v = (
            self.wv(x).view(B, T, self.n_kv_head, self.d_head).transpose(1, 2)
        )  # (B,Hk,T,D)
        ...
        if self.sliding_window is not None and k_all.size(2) > (
            self.sliding_window + self.attention_sink
        ):
            s = self.attention_sink
            k_all = torch.cat(
                [k_all[:, :, :s, :], k_all[:, :, -self.sliding_window :, :]], dim=2
            )
            v_all = torch.cat(
                [v_all[:, :, :s, :], v_all[:, :, -self.sliding_window :, :]], dim=2
            )
