...
def main():
    ...
    p.add_argument(
        "--bpe", action="store_true", help="train and use a BPE tokenizer (recommended)"
    )
    ...
    p.add_argument(
        "--steps", type=int, default=300, help="max optimizer steps for this run"
    )
    ...
    # misc
    p.add_argument(
        "--log", choices=["wandb", "tensorboard", "none"], default="tensorboard"
    )
    p.add_argument(
        "--save_every",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=2,
        help="keep last K step checkpoints (plus model_last.pt)",
    )
    args = p.parse_args()
    ...
    train_loader = make_loader(
        args.data, tok, args.block_size, args.batch_size, shuffle=True
    )
    ...
    sched = WarmupCosineLR(
        optim, warmup_steps=warmup, total_steps=total_steps, base_lr=args.lr
    )
